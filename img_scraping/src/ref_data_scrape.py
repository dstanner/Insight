...
totalwine_base_url = ('https://www.totalwine.com/spirits/c/',
                      'c0030?tab=fullcatalog&viewall=true&text=',
                      '&pagesize=180&page=')
totalwine_p_num = 86

page_url_list = [totalwine_base_url+str(i) for
                 i in range(1, totalwine_p_num+1)]
...
totalwine_complete_item_urls = [totalwine_add_prefix + url
                                for url in totalwine_item_urls]

# Get rid of duplicaes
totalwine_item_urls = list(set(totalwine_complete_item_urls))

# Get rid of bot checker
totalwine_item_urls = [x.replace('&igrules=true', '') for
                       x in totalwine_item_urls]
...
for page in totalwine_item_urls:
    ...
    try:
        ...
        container = curr.find(class_=['pdp-img-zoom-modal-zoom-reset',
                                      'anPDPImage'])
...
totalwine_img_urls['img_url'] = ('https://www.totalwine.com/' +
                                 totalwine_img_urls['img_url'].astype(str))
...
for i in range(len(totalwine_image_urls)):
    ...
    if totalwine_image_urls['img_url'].iloc[i] not in totalwine_skip_urls:
        ...
        try:
            img_data = requests.get(totalwine_image_urls['img_url'].
                                    iloc[i]).content
...
liquormart_page_url_list = [liquormart_base_url+str(i) for i in
                            range(1, liquormart_n_pages+1)]
...
for page in liquormart_item_urls:
    ...
    try:
        ...
        curr_info['img_url'] = (curr.find(attrs=
